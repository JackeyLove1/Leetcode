...
int main() {
    fhj();
    cin >> n;
    set<int> splay1, splay2;
    ll res = 0;
    for (int i = 1; i <= n; ++i) {
        int x;
        cin >> x;
        if (splay1.empty() && splay2.empty()) {
            res += x;
            splay1.insert(x);
            splay2.insert(-x);
        } else if (splay1.count(x)) {
            continue;
        } else {
            auto next = lower_bound(splay1.begin(), splay1.end(), x);
            auto prev = lower_bound(splay2.begin(), splay2.end(), -x);
            auto v = INF;
            if (prev != splay2.end()) {
                v = std::min(v, std::abs(-x - *prev));
            }
            if (next != splay1.end()) {
                v = std::min(v, std::abs(x - *next));
            }
            // cout <<"x: " << x << " prev: " << -(*prev) <<" next: " << *next << endl;
            res += v;
            splay1.insert(x);
            splay2.insert(-x);
        }
